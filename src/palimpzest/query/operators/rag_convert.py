...
class RAGConvert(LLMConvert):
    def __init__(self, num_chunks_per_field: int, chunk_size: int = 1000, *args, **kwargs):
        ...
        self.client = None
        self.embedding_model = Model.VLLM_NOMIC_EMBED_TEXT_V1
        ...
        for field_name in input_fields:
            ...
            if is_list_string_field:
                ...
                if value is None:
                    ...
                    candidate[field_name] = "[" + ", ".join(map(str, value)) + "]"
    ...
    def convert(self, candidate: DataRecord, fields: dict[str, FieldInfo]) -> tuple[dict[str, list], GenerationStats]:
        ...
        self.client = OpenAI(base_url="http://localhost:8001/v1", api_key="EMPTY") if self.client is None else self.client
